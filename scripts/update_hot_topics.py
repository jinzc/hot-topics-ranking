...
"""
全网热点榜单自动更新脚本
每小时自动抓取微博、百度、知乎等平台热搜数据，生成 hot_data.json
"""
...
# 分类关键词映射
CATEGORY_KEYWORDS = {
    'politics': ['特朗普', '普京', '外交部', '中美', '会晤', '访问', '政治', '外交', '军事', '战争', '伊朗', '以色列'],
    'tech': ['微信', 'AI', '人工智能', '芯片', '华为', '苹果', '三星', '蓝牙', '科技', '数码', '手机', '电脑', '互联网'],
    'entertainment': ['明星', '演员', '电影', '电视剧', '综艺', '演唱会', '李连杰', '迪士尼', '娱乐', '艺人', '导演'],
    'sports': ['世界杯', '足球', '篮球', 'NBA', '乒乓球', '体育', '比赛', '冠军', '世乒赛', '运动员'],
    'finance': ['股价', '股市', '经济', '金融', '银行', '存款', '油价', '房价', '任天堂', '卡乐比', '企业', '公司'],
    'health': ['疫情', '病毒', '疫苗', '医院', '医生', '健康', '疾病', '药品', '医疗', '养生'],
    'life': ['机票', '旅游', '酒店', '美食', '购物', '价格', '生活', '日常', '家庭', '婚姻', '育儿'],
    'social': ['地震', '火灾', '事故', '灾难', '救援', '警察', '法院', '法律', '社会', '民生', '公益']
}
...
def merge_topics(weibo_data, baidu_data, zhihu_data):
    """合并各平台数据，去重并计算综合热度"""
    ...
    for item in zhihu_data:
        key = item['title'][:20]
        if key in topic_map:
            topic_map[key]['heat']['zhihu'] = item['heat']
            topic_map[key]['sources'].append({'name': '知乎', 'url': item['url']})
        else:
            topic_map[key] = {
                'title': item['title'],
                'heat': {'weibo': 0, 'baidu': 0, 'douyin': 0, 'bilibili': 0, 'zhihu': item['heat']},
                'sources': [{'name': '知乎', 'url': item['url']}],
                'summary': '',
                'rank': item['rank']
            }
...
def generate_data():
    ...
    # 合并去重
    topics = merge_topics(weibo_data, baidu_data, zhihu_data)
